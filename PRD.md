...
| 日期 | 2026-06-14 |
| 状态 | Draft（待评审后进入实现） |
| 前身 | `loudness_dd`（tabCapture 方案,因 MV3 限制已废弃,见 §3） |
...
平衡算法采用广播标准 **ITU-R BS.1770 LUFS**(从 loudness_dd 复用),目标是让每个标签页的 short-term LUFS 收敛到用户设定值(默认 -14 LUFS)。
...
### 3.1 loudness_dd 的方案与失败

前身 `loudness_dd` 使用 MV3 的 `chrome.tabCapture.getMediaStreamId({ targetTabId })` 捕获标签页音频,在 offscreen document 里跑 LUFS + GainNode。

**这个方案在 MV3 下有一个无法绕过的根本限制。** Chrome 官方文档(`developer.chrome.com/docs/extensions/reference/api/tabCapture`)明确写道:
...
- **测量分支**:并联一个 `AudioWorkletNode`(复用 loudness_dd 的 `lufs-processor.ts`),输出静音,只为算 LUFS。
...
#### 6.2.2 平衡决策(复用 loudness_dd 的纯函数)

直接复用 `computeBalanceGains(tabs, target, soloTabId)`(已单元测试,见 §17)。每次收到 `LUFS_REPORT` 触发一次决策(带 100ms 节流,与 loudness_dd 一致)。

#### 6.2.3 持久化与 SW 休眠

MV3 service worker 会休眠,内存状态丢失。设计原则(从 loudness_dd 踩坑学来):
...
### 6.3 Popup UI(Vue 3 + Pinia,复用 loudness_dd)
...
**复用** loudness_dd 的 `computeBalanceGains(tabs, target, soloTabId)`:
...
**移除**(相对 loudness_dd):`tabCapture`、`activeTab`、`offscreen`——content script 方案完全不需要。
...
| 模块 | 测试 |
|---|---|
| `computeBalanceGains` | 复用 loudness_dd 的 12 个用例(各种 clamp/skip/solo) |
| `LufsCalculator` | 复用 loudness_dd 的 24 个用例(K-weighting、gating、block cap) |
...
真实浏览器验证(无法自动化 DRM 等):
- 准备测试音源(从 loudness_dd 复用 `tools/loudness-test.html`,粉噪 + 可控增益)。
...
| i18n | vue-i18n(中/英,从 loudness_dd 复用) |
...
## 15. 从 loudness_dd 复用的资产

直接复制 + 小改:

| 文件 | 复用方式 |
|---|---|
| `src/audio/lufs.ts` | 原样复用(BS.1770 算法) |
| `src/worklets/lufs-processor.ts` | 原样复用 |
| `src/audio/balance.ts` | 原样复用(`computeBalanceGains` 等纯函数) |
| `src/components/AutoBalance.vue` | 原样复用 |
| `src/components/Limiter.vue` | 原样复用 |
| `src/components/TabList.vue` | 复用 + 加 gain 实时显示 |
| `src/stores/settings.ts` | 原样复用(locale 持久化) |
| `src/stores/tabs.ts` | 改造(适配新协议) |
| `src/i18n.ts` + `locales/` | 原样复用 |
| `src/__tests__/lufs-calculator.spec.ts` | 原样复用(24 用例) |
| `src/__tests__/lufs-processor.spec.ts` | 原样复用(7 用例) |
| `src/__tests__/balance.spec.ts` | 原样复用(12 用例) |
| `src/__tests__/components/*.spec.ts` | 复用 + 适配 |
| `tools/loudness-test.html` | 原样复用(测试音源) |
| `vitest.config.ts` / `tsconfig` 模式 | 参考复用 |

**不复用**(架构相关,重写):
- `src/background.ts`(tabCapture 逻辑全废弃,新协调器)
- `src/offscreen.ts`(整个 offscreen 层废弃)
- `manifest.config.ts`(新权限模型)
...
1. **Limiter 是否保留?** loudness_dd 有输出限幅器。content script 方案下,gain 可能 > 1 导致削波。建议**默认开启 limiter**(threshold -1 dB),防提升后削波。待评审。
...
## 附录 B:与 loudness_dd 的差异对照

| 维度 | loudness_dd | EqualLoud |
|---|---|---|
| 音频接管方式 | tabCapture 录制 | createMediaElementSource 拦截 |
| 运行位置 | offscreen document | 页面 content script |
| 自动生效 | ❌ 需用户点图标 | ✅ 完全自动 |
| 需要 activeTab | ✅(死路) | ❌ |
| 需要 host_permissions | 部分 | ✅(注入) |
| DRM 站点 | 可捕获(但有限制) | 静音,需降级 |
| 浏览器重启 | 失效 | 自动恢复 |
| 多 tab 平衡 | 同 | 同(SW 协调) |
| 增益范围 | 任意 gain | 任意 gain(>1 放大) |
| 平滑过渡 | setTargetAtTime | 同 |
| LUFS 算法 | BS.1770 | 同(复用) |
| popup UI | Vue 3 | 同(复用) |
